...
14. Applying the nearby People filter narrows only the People dots. Sold Property pins that satisfy the current date filters stay visible through nearby-controller edits, a Cancel button appears next to Apply nearby filter while the filter is active, and changing nearby-controller inputs or applying/canceling the nearby filter does not reset the map to the default Highland Park view.
