...
- When nearby People rows are listed, `Export CSV` downloads `First Name,Mobile Phone,Address` rows using the selected Sold Property address and same-suburb mode in the filename. The Address column combines street address and suburb separated by a comma.
