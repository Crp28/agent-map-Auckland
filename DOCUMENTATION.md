...
- 2026-05-12: Added optional `preferred_name` support to People records, updated the multi-address repository/UI/import flows to preserve legal name plus preferred first name, and taught the PropertySmarts owner checker to compare against both the legal full name and the preferred-first-name display variant.
- 2026-05-26: Tightened preferred-name semantics so `preferred_name` is treated as a preferred first name only. Imports and saves now normalize it to one token, UI display derives the full visible name from legal surname/rest-of-name, and owner matching now compares against the derived preferred display form instead of a bare first name.
