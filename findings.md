...
- The current owner-check mismatch class is not always a scrape bug. Contact-import/manual-entry flows were collapsing `Preferred Name` into the only People `name` field, which loses the legal-name variant needed for title-owner comparison.
...
- People records need separate legal-name and preferred-name fields. The preferred-name field is now defined as a preferred first name only, so UI display and owner matching must derive a full visible name by combining that preferred first name with the legal surname/rest-of-name.
...
- GeoMaps misses should not automatically trigger a slower secondary provider on every save. The cleaner boundary is: save normally with GeoMaps, detect unresolved results in the API response, ask the user whether to try Google Maps, and patch only the unresolved record if they agree.
